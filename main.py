...
# ==========================================
st.set_page_config(page_title="CAM DXF Control Pro AI", layout="wide", page_icon="🛠️")
...
# ==========================================
# LOẠI 1 & 2: AI DXF & RISK ANALYSIS
# ==========================================
def ai_dxf_and_risk_analysis(api_key, detected_parts, material, tool_dia, feed_rate, cut_z):
    if not HAS_OPENAI:
        return "❌ Thư viện `openai` chưa được cài đặt trong `requirements.txt`!"
    if not api_key:
...
# ==========================================
st.title("🛠️ CAM DXF Control Dashboard Pro (AI-Powered)")
...
with col_display:
    if uploaded_file is not None:
        try:
            ...
            # TABS CHỨC NĂNG
            tab_sim, tab_gcode, tab_ai = st.tabs(["📊 Mô phỏng 2D & Work Zero", "💾 Xuất G-Code", "🤖 AI & Machine Learning Center"])
            ...
            # TAB 3: TRUNG TÂM AI & MACHINE LEARNING
            with tab_ai:
                st.subheader("🤖 AI Control & Optimization Center")
                ...
                st.markdown("### 🔍 2. AI Cảnh báo Lỗi DXF & Chế độ Cắt (OpenAI API)")
                api_key_input = st.text_input("Nhập OpenAI API Key:", type="password", help="sk-proj-...")
                ...
                if st.button("🚀 Chạy kiểm tra AI Toàn diện", type="primary"):
                    with st.spinner("AI đang phân tích file DXF và tính toán chế độ cắt..."):
